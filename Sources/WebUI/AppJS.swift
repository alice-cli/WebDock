...
function setupH264Decoder(m){
  ...
  try {
    h264Codec = m.codec || 'avc1.4D401F';
    h264W = m.width | 0;
    h264H = m.height | 0;
    h264Desc = m.description ? b64ToU8(m.description) : null;
    h264WaitingKey = true;
    h264Ts = 0;
    if (h264Decoder) {
...
function handleH264Sample(u8){
  ...
  try {
    // Keep decode queue tiny but not zero (0→underrun choppy, 3+→lag).
    const q = h264Decoder.decodeQueueSize;
    if (q > 2 && !key) { _statDrops++; return; }
    if (q > 5) { _statDrops++; return; }
    h264Ts += 33333;
...
setInterval(() => {
  ...
  let pressure = 0;
  if (queue >= 3 || drops > 12 || fps < 12) pressure = 3;
  else if (queue >= 2 || drops > 5 || fps < 18) pressure = 2;
  else if (queue >= 1 || drops > 2 || fps < 24) pressure = 1;
...
function setPreset(name){
  ...
  ['preFast','preBal','preLive'].forEach(id => {
    const el = document.getElementById(id);
    if (!el) return;
    const on = (id==='preFast' && n==='fast') || (id==='preBal' && n==='balanced') || (id==='preLive' && n==='broadcast');
