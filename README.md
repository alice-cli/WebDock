...
**UI languages:** English · 한국어 · 日本語 · 中文 · Deutsch · Français  
(웹 상단 언어 메뉴에서 선택 · 브라우저 언어 자동 감지)

- Host: **macOS 14+**
- Client: latest Chrome / Edge / Safari / Firefox
- Source: Swift Package Manager (no external package dependencies)

---

## English (quick)

Remote-control a Mac window from any browser. Stream via ScreenCaptureKit, inject mouse/keyboard, optional token auth and LAN access.

**Install (notarized):** [Releases](https://github.com/alice-cli/WebDock/releases) → download `WebDock-macOS-*.zip` → open `WebDock.app` → enable **Screen Recording** + **Accessibility**.

**Build from source:** `git clone … && ./install_home.sh`  
**Permissions stick across rebuilds:** `./setup_dev_cert.sh` (local cert, free — not Apple Developer Program).

**Covered windows:** WebDock raises the target app when another window is in front so clicks/keys still work.
...
| **가려진 창 제어** | 다른 앱이 위에 있어도 대상 창을 앞으로 올린 뒤 입력 |
| 웹 UI 다국어 | EN / KO / JA / ZH / DE / FR |
| 화질 | 프리셋 + JPG / PNG / H.264 |
...
| Gatekeeper 경고 없이 **다운로드 배포** (공증) | **예** (Apple Developer Program) | [Releases](https://github.com/alice-cli/WebDock/releases) 의 zip은 CI에서 공증됨 |
...
선택 값은 브라우저 `localStorage`에 저장됩니다.

### 가려진 창

대상 창 위에 다른 앱이 있으면, 클릭/키 입력 전에 **대상 앱을 앞으로 올립니다.**  
손쉬운 사용 권한이 필요합니다.
